...
深度学习方面，采用改造 ResNet 骨干（$3{\times}3$ 首层卷积、Huber Loss、Mixup 数据增强）训练 CNN 回归模型，推理阶段通过 MC Dropout 提供不确定性估计。在公平对比评测中（CNN 用 $128{\times}128$ 训练裁剪，FFT 用 $512{\times}512$ 重算高度图），FFT 平均误差更低（MAE $\approx 0.027^{\circ}$ vs.\ CNN $\approx 0.053^{\circ}$），但 CNN 在低分辨率端到端推理、畸变鲁棒性及不确定性量化方面具有不可替代的优势，二者形成互补。
...
需要强调的是，上述对比基于仿真数据。真实 AFM 图像可能包含晶界、污染、多层堆叠等非周期性缺陷，这些因素对 FFT 峰值检测的破坏可能远大于对 CNN 的影响——CNN 在训练过程中已见过多种退化模式的联合分布。真实数据上的最终优劣仍需后续实验验证。
...
\begin{generaltab}{退化鲁棒性扫描汇总（中位误差，$^{\circ}$）}{tab:robustness}
  \begin{tabularx}{\textwidth}{llCCCC}
    ...
    \midrule
    \multirow{3}{*}{噪声} & 0.05（分布内） & 0.003 & 0.041 & 0.08 & in-distr \\
    & 0.15（分布上界） & 0.003 & 0.070 & 0.05 & in-distr \\
    & 0.50（OOD, 3.3$\times$） & 0.003 & 1.589 & 0.002 & OOD \\
    \midrule
    \multirow{3}{*}{模糊} & 0.5（分布上界） & 0.003 & 0.045 & 0.07 & in-distr \\
    & 2.0（OOD, 4$\times$） & 0.003 & 0.039 & 0.08 & OOD \\
    & 5.0（OOD, 10$\times$） & 0.008 & 0.312 & 0.02 & OOD \\
    \midrule
    \multirow{3}{*}{仿射畸变} & 0.01（分布内） & 0.004 & 0.045 & 0.08 & in-distr \\
    & 0.05（OOD, 3.3$\times$） & 0.003 & 0.033 & 0.08 & OOD \\
    & 0.25（OOD, 16.7$\times$） & 0.013 & 0.062 & 0.20 & OOD \\
    \midrule
    \multirow{3}{*}{探针卷积} & 7.0（分布上界） & 0.003 & 0.037 & 0.09 & in-distr \\
    & 15.0（OOD, 2.1$\times$） & 0.007 & 0.071 & 0.10 & OOD \\
    & 30.0（OOD, 4.3$\times$） & 0.125 & 0.527 & 0.24 & OOD \\
    \bottomrule
  \end{tabularx}
\end{generaltab}
...
FFT 唯一出现明显误差退化的场景是极端探针卷积（tip=30 nm，MAE 从 $0.003^{\circ}$ 升至 $0.125^{\circ}$），因为探针半径接近 moiré 周期时会严重模糊条纹的空间结构。相比之下，CNN 在高噪声（noise=0.5）和高探针卷积（tip=20 nm 以上）场景下误差急剧增大至 $0.3^{\circ}$--$1.6^{\circ}$，说明 CNN 对超出训练分布的退化缺乏外推能力。

基于上述全面评测，在当前仿真数据体系下，FFT 是转角提取的最优方法——无论是绝对精度（MAE $0.027^{\circ}$）还是退化鲁棒性均全面优于 CNN。CNN 的潜在价值体现在：推理速度（$128{\times}128$ vs $512{\times}512$）、端到端 simplicity、MC Dropout 不确定性量化（$\pm 0.176^{\circ}$，$93.5\%$ 在 $1\sigma$ 内），以及面向真实实验数据迁移的可能性。
...
(3) 构建了面向轻敲模式多通道、含探针卷积与多种电学/力学退化的仿真数据集（$N=50000$，三通道 $128{\times}128$），并采用改造 ResNet18 + Huber 损失 + Mixup 数据增强策略训练 CNN。在公平对比（paired 口径，CNN 用 $128^2$、FFT 用 $512^2$ 同场景）下，全量评测结果为 FFT MAE $0.027^{\circ}$、CNN MAE $0.053^{\circ}$，FFT 平均误差约为 CNN 的一半。CNN 可给出 MC Dropout 不确定性（平均 $\pm 0.196^{\circ}$，$93.5\%$ 在 $1\sigma$ 内），基于不确定性的动态融合可将 MAE 进一步降至 $0.042^{\circ}$。

(4) 通过四维度退化鲁棒性扫描（噪声、模糊、仿射畸变、探针卷积），从训练分布内推到分布外，系统量化了两种方法的鲁棒性边界。结果表明 FFT 在所有退化维度上均保持极高鲁棒性（即使在 3--17 倍于训练分布的极端退化下，FFT MAE 仍 $\leq 0.013^{\circ}$），其物理机制在于频域峰值位置不受退化幅度的直接影响。CNN 在超出训练分布的退化条件下误差急剧增大，但 FFT 在极端探针卷积（tip=30 nm）下也会出现明显退化（MAE $0.125^{\circ}$）。综合精度与鲁棒性，FFT 是当前仿真数据体系下的最优方法。
