...
\input{../../angle_cnn/outputs/thesis/thesis_tab_eval_modes}

表~\ref{tab:final_stats} 给出 paired 口径下的完整误差统计。

\input{../../angle_cnn/outputs/thesis/thesis_tab_final_stats}
...
\textbf{(b) 不确定性量化。} FFT 返回单一角度值，不提供置信度信息，使用者无法判断特定预测是否可靠。CNN 通过 MC Dropout 采样（本文 30 次）输出标准差，本实验中 $92.1\%$ 的预测落在 $1\sigma$ 区间内，可有效标记异常预测。
...
\begin{generalfig}[htbp]{FFT（左）与 CNN（右）预测值 vs 真实值，融合方法（右）}{fig:compare_scatter}
  \includegraphics[width=\textwidth]{thesis_fig_scatter.png}
\end{generalfig}
...
\begin{generalfig}[htbp]{误差分布直方图}{fig:error_dist}
  \includegraphics[width=\textwidth]{thesis_fig_error_dist.png}
\end{generalfig}

图~\ref{fig:error_dist} 给出误差分布直方图，可见 FFT 误差集中在 $0^{\circ}$ 附近，而 CNN 误差分布更宽。

\begin{generalfig}[htbp]{各角度区间 MAE 对比}{fig:angle_bin}
  \includegraphics[width=\textwidth]{thesis_fig_angle_bin_mae.png}
\end{generalfig}

图~\ref{fig:angle_bin} 和表~\ref{tab:angle_bins} 展示了按角度分箱的 MAE 对比。FFT 在所有角度区间上均优于 CNN，且小角度（$< 2^{\circ}$）的优势更为显著。

\input{../../angle_cnn/outputs/thesis/thesis_tab_angle_bins}

\begin{generalfig}[htbp]{CNN 不确定性校准曲线}{fig:calibration}
  \includegraphics[width=\textwidth]{thesis_fig_calibration.png}
\end{generalfig}

图~\ref{fig:calibration} 展示了 CNN 的 MC Dropout 不确定性校准曲线，表~\ref{tab:calibration} 汇总了校准指标。$1\sigma$ 覆盖率达到 $92.1\%$，说明 CNN 的不确定性估计整体可靠。

\input{../../angle_cnn/outputs/thesis/thesis_tab_calibration}

\subsection{多维度退化鲁棒性扫描}

为量化两种方法在不同退化条件下的鲁棒性边界，设计了四维退化扫描实验（\texttt{robustness\_sweep.py}）：高斯噪声、高斯模糊、仿射畸变和探针卷积。每种退化从训练分布内逐步推到训练分布外（OOD），固定其他退化参数为训练分布中位数，仅变化目标维度。每个条件在 5 个测试角度（$0.5^{\circ}$--$5.0^{\circ}$）上各重复 10 次试验，CNN 启用 30 次 MC Dropout 采样。结果如图~\ref{fig:robustness_sweep} 和表~\ref{tab:robustness_p1}、\ref{tab:robustness_p2}。
...
\input{../../angle_cnn/outputs/thesis/thesis_tab_robustness_p1}
\input{../../angle_cnn/outputs/thesis/thesis_tab_robustness_p2}
