...
深度学习方面，采用改造残差网络骨干（$3{\times}3$ 首层卷积、Huber 损失、Mixup 数据增强）训练卷积神经网络回归模型，推理阶段通过蒙特卡洛 Dropout 提供不确定性估计。在公平对比评测中（卷积神经网络用 $128{\times}128$ 训练裁剪，快速傅里叶变换用 $512{\times}512$ 重算高度图），快速傅里叶变换平均误差更低（平均绝对误差 $\approx 0.027^{\circ}$ vs.\ 卷积神经网络 $\approx 0.053^{\circ}$），这是仿真数据保持完整周期性的预期结果。卷积神经网络虽在仿真精度上不及快速傅里叶变换，但具备物理标定无关、内置不确定性量化以及端到端自动化推理的优势，在真实实验数据上可能展现出不同的优劣格局，二者形成互补。
