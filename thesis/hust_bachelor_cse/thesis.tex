...
深度学习方面，采用改造 ResNet 骨干（$3{\times}3$ 首层卷积、Huber Loss、Mixup 数据增强）训练 CNN 回归模型，推理阶段通过 MC Dropout 提供不确定性估计。在公平对比评测中（CNN 用 $128{\times}128$ 训练裁剪，FFT 用 $512{\times}512$ 重算高度图），FFT 平均误差更低（MAE $\approx 0.028^{\circ}$ vs.\ CNN $\approx 0.047^{\circ}$），但 CNN 在低分辨率端到端推理、畸变鲁棒性及不确定性量化方面具有不可替代的优势，二者形成互补。
...
\begin{generaltab}{两种评测口径结果对比（$N=5000$）}{tab:eval_modes}
  \begin{tabularx}{\textwidth}{lCCCC}
    ...
    \midrule
    MAE ($^{\circ}$) & $0.0278$ & $0.0274$ & $0.0516$ & $0.0513$ \\
    P90 ($^{\circ}$) & $0.0647$ & $0.0630$ & $0.1119$ & $0.1117$ \\
    最大误差 ($^{\circ}$) & $0.1442$ & $0.1450$ & $0.4183$ & $0.3556$ \\
    MAE 比值 (FFT/CNN) & $0.539$ & $0.534$ & --- & --- \\
    \bottomrule
  \end{tabularx}
\end{generaltab}
...
\begin{generaltab}{FFT vs CNN 完整误差统计（paired 口径，$N=5000$）}{tab:final_stats}
  \begin{tabularx}{\textwidth}{lCC}
    ...
    有效样本数 & 5000 & 5000 \\
    失败率 & $0.0\%$ & $0.0\%$ \\
    MAE ($^{\circ}$) & $0.0274$ & $0.0513$ \\
    中位误差 ($^{\circ}$) & $0.0197$ & $0.0391$ \\
    标准差 ($^{\circ}$) & $0.0248$ & $0.0452$ \\
    90 百分位误差 ($^{\circ}$) & $0.0630$ & $0.1117$ \\
    最大误差 ($^{\circ}$) & $0.1450$ & $0.3556$ \\
    \midrule
    MC Dropout 平均不确定度（CNN） & --- & $\pm 0.176^{\circ}$ \\
    1$\sigma$ 覆盖率（CNN） & --- & $93.5\%$ \\
    融合 MAE ($^{\circ}$) & \multicolumn{2}{c}{$0.0420$（$\tau = 0.5^{\circ}$）} \\
    \bottomrule
  \end{tabularx}
\end{generaltab}

在该公平设定下，FFT 的平均误差约为 CNN 的一半（MAE 比值 $0.534$）。这一结果符合预期：仿真数据保留了 moiré 条纹的完整周期性，FFT 的频域峰值检测恰好是物理模型的逆运算，因此在理想条件下具有天然精度优势。CNN 的输入分辨率仅为 $128{\times}128$（16K 像素），频率信息受限，但作为端到端方法仍能达到 $0.051^{\circ}$ 的 MAE。
...
(3) 构建了面向轻敲模式多通道、含探针卷积与多种电学/力学退化的仿真数据集（$N=50000$，三通道 $128{\times}128$），并采用改造 ResNet18 + Huber 损失 + Mixup 数据增强策略训练 CNN。在公平对比（paired 口径，CNN 用 $128^2$、FFT 用 $512^2$ 同场景）下，全量评测结果为 FFT MAE $0.027^{\circ}$、CNN MAE $0.051^{\circ}$，FFT 平均误差约为 CNN 的一半。CNN 可给出 MC Dropout 不确定性（平均 $\pm 0.176^{\circ}$，$93.5\%$ 在 $1\sigma$ 内），基于不确定性的动态融合可将 MAE 进一步降至 $0.042^{\circ}$。
...
中位误差 (°)                   0.0197       0.0391
std (°)                    0.0248       0.0452
90th pct (°)               0.0630       0.1117
最大误差 (°)                   0.1450       0.3556

MAE 比值 MAE_FFT / MAE_CNN = 0.534
