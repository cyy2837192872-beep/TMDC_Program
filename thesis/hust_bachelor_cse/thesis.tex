...
深度学习方面，采用改造 ResNet 骨干（$3{\times}3$ 首层卷积、Huber Loss、Mixup 数据增强）训练 CNN 回归模型，推理阶段通过 MC Dropout 提供不确定性估计。在公平对比评测中（CNN 用 $128{\times}128$ 训练裁剪，FFT 用 $512{\times}512$ 重算高度图），FFT 平均误差更低（MAE $\approx 0.027^{\circ}$ vs.\ CNN $\approx 0.053^{\circ}$），这是仿真数据保持完整周期性的预期结果。CNN 虽在仿真精度上不及 FFT，但具备物理标定无关、内置不确定性量化以及端到端自动化推理的优势，在真实实验数据上可能展现出不同的优劣格局，二者形成互补。
