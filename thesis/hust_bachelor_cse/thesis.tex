...
\date{\today}
\school{物理学院}
\classnum{xxxxxxxx}          % 填写专业班级
\stunum{xxxxxxxxxx}          % 填写学号
\instructor{张卜天}
...
\begin{generaltab}{FFT vs CNN 误差统计（公平对比 v3，$N=5000$，数据来自 \texttt{compare\_report.txt}）}{tab:final_stats}
  \begin{tabularx}{\textwidth}{lCC}
    ...
    有效样本数 & 5000 & 5000 \\
    失败率 & $0.0\%$ & $0.0\%$ \\
    MAE ($^{\circ}$) & $\bm{0.0315}$ & $0.0810$ \\
    中位误差 ($^{\circ}$) & $0.0147$ & $0.0474$ \\
    标准差 ($^{\circ}$) & $0.0435$ & $0.0879$ \\
    90 百分位误差 ($^{\circ}$) & $0.0829$ & $0.2112$ \\
    最大误差 ($^{\circ}$) & $0.3494$ & $0.5591$ \\
    \midrule
    MC Dropout 平均不确定度（CNN） & --- & $\pm 0.156^{\circ}$（示例） \\
    \bottomrule
  \end{tabularx}
\end{generaltab}

在该公平设定下，FFT 的平均误差低于 CNN；CNN 仍可提供不确定性估计，并在仅允许小图输入或畸变 sweep 等场景具有补充价值，真实 AFM 数据上的优劣需待实验后结论。
...
(3) 构建了面向轻敲模式多通道、含探针卷积与多种电学/力学退化的仿真数据集，并采用改造 ResNet + Huber/Mixup 等策略训练 CNN；在公平对比（CNN 用 $128^2$、FFT 用 $512^2$ 重算）下，当前典型结果为 FFT MAE 约 $0.032^{\circ}$、CNN MAE 约 $0.08^{\circ}$，即该设定下 FFT 平均误差更低；CNN 仍可给出 MC Dropout 不确定性，并在畸变扫描等补充实验中与 FFT 形成对照。
...
中位误差 (°)                   0.0147       0.0474
std (°)                    0.0435       0.0879
90th pct (°)               0.0829       0.2112
最大误差 (°)                   0.3494       0.5591

CNN 比 FFT 精度提升：0.4 倍
