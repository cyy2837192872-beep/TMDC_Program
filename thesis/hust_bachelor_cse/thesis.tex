...
\textbf{(1) AFM 记录的是什么。}
轻敲（Tapping）模式下，针尖在 $x$--$y$ 扫描面上逐点振动，反馈维持设定振幅，主要输出 \textbf{Height} 通道 $z(x,y)$，并常同步采集 \textbf{Phase}、\textbf{Amplitude}。Height 图像是\textbf{二维平面上的标量场}：每个像素对应局域针尖位置或等效高度，\textbf{并非}沿 $z$ 方向测量倒格矢；moiré 超周期的\textbf{横向重复尺度}体现在 $x$--$y$ 面内条纹/畴结构的周期。Phase、Amplitude 往往对耗散、黏弹与刚度敏感，\textbf{不一定}等同于纯几何高度，但与 Tapping 实验常见导出量一致，故本文仿真保留三通道以利于与实测形态对齐。

\textbf{(2) ``平整''样品上为何仍有高度起伏。}
少层二维材料在宏观上极薄，但在 AFM 纳米分辨率下并非理想无穷平面：衬底起伏、转移应变与褶皱、以及转角体系中\textbf{堆叠位形随位置变化}引起的局域弛豫等，均可带来面外形貌或表观高度对比；仪器扫描还会引入残余倾斜、行纹与噪声（仿真中以退化链模拟）。因此 Height 图上的明暗起伏应理解为\textbf{局域形貌与针尖--样品相互作用在反馈下的综合结果}，而非单一``理想平面 + 常数 $z$''。

\textbf{(3) 针尖曲率半径的影响。}
针尖具有有限曲率半径，与样品发生\textbf{横向卷积}，使空间尺度小于针尖尺度的细节被展宽、平滑；当 moiré 周期与针尖尺度可比时条纹对比度显著下降。这与第~3~章表~\ref{tab:degradation}~中的探针卷积模块及本节上文 FFT 适用边界(2)的论述一致。

\textbf{(4) 转角 $\theta$ 到 Height 像的逻辑链。}
转角首先通过式~\eqref{eq:period} 决定 moiré 周期 $L(\theta)$，从而改变\textbf{面内}堆叠织构的横向重复尺度；连续晶格重构强度式~\eqref{eq:strength} 随 $\theta$ 变化，调制畴结构与式~\eqref{eq:recon} 给出的强度分布。故改变 $\theta$ 将\textbf{整体改变}仿真或实验中观察到的图案尺度与对比度。需区分：$L(\theta)$ 刻画的是 moiré 在\textbf{横向上}的重复距离；若存在堆叠依赖的面外弛豫，其\textbf{峰谷幅度}由弹性能与界面相互作用决定，\textbf{不能}由 $L$ 单独唯一确定。

\textbf{(5) 本文仿真图像的定位。}
第~3~章中由程序生成的 height / phase / amplitude 是在已知 $\theta$ 下由物理模型与退化链构造的\textbf{合成强度场}，用于算法训练与评测，\textbf{意在逼近} Tapping 模式常见通道的信息形态；\textbf{不等同于}单次实验的原始读出或第一性原理逐点面位移场。与实测对比时应考虑 domain gap，并以 SHG 等独立手段作为转角真值（第~4~章）。
...
三通道仿真样本是在已知标签 $\theta$ 下由第~2~章模型与退化链构造的\textbf{合成图像}：其 height / phase / amplitude 与真实 AFM 导出量在\textbf{信息形态}上对齐，用于训练与算法验证；\textbf{物理定量}（如面外位移的亚埃数值）应以实验或第一性原理为准。相关物理解释见第~\ref{subsec:afm_height_explain}~节。
...
实验所得到的 Height 等通道与本文仿真图像在\textbf{定义与局限}上的对应关系，已集中在第~\ref{subsec:afm_height_explain}~节说明：尤其应注意 Height 为 $z(x,y)$ 标量场而非``$z$ 向倒格''测量，且针尖尺度与 moiré 周期同量级时会降低条纹对比度。
