...
% 项目根目录下由脚本生成的图（在 hust_bachelor_cse 目录执行 xelatex/lualatex）
\graphicspath{{../../angle_cnn/outputs/}{../../data/}{Figures/}}
...
转角二维材料体系通过引入层间转角形成 moiré 超晶格，在特定"魔角"附近出现平带与强关联效应，是凝聚态物理的重要研究前沿。MoS$_2$ 转角同质结作为该体系的典型代表，其物理性质对转角精度极为敏感，精准提取转角因此成为制备与表征的核心挑战。传统基于快速傅里叶变换（FFT）的方法在频率分辨率充足、畸变较小时精度优良，但在小视野或强畸变条件下峰检测易失效；深度学习方法可在固定输入尺寸下做端到端回归，并与 Monte Carlo Dropout 等结合给出不确定性，便于与实验流程衔接。

本文建立了一套从物理仿真到深度学习的 MoS$_2$ moiré 转角自动提取 pipeline。仿真上，基于 moiré 倒格矢差模型生成包络，并引入连续晶格重构（$\mathrm{strength} = \mathrm{clip}(1 - \theta/2,\, 0,\, 1)$）描述 AB/BA 畴过渡。数据集面向轻敲模式（Tapping mode）多通道（height / phase / amplitude）成像，默认 $128{\times}128$ 像素裁剪与约 $10^4$--$10^5$ 级样本规模，并在退化链中纳入 TITAN~70 类探针卷积、仿射畸变、背景倾斜、行噪声、高斯模糊、$1/f$ 噪声及反馈振荡等，以逼近 Cypher ES 等 AFM 实验条件。模型采用改造的 ResNet 骨干（$3{\times}3$ 首卷积、移除不当 max pooling、Huber $\delta=0.02$、Mixup 等），并可选用 FFT 幅值作为附加通道及 MC Dropout 不确定性估计。

评测方面强调``公平对比''：CNN 始终作用于与训练一致的 $128{\times}128$ 三通道图；FFT 则在按相同物理与退化条件重算得到的 $512{\times}512$ 高度图上提取，以避免小裁剪视野导致的频率分辨率不足。当前仓库脚本 \texttt{eval\_compare.py}（v3）在测试集（如 $N=5000$）上给出的典型结果为：FFT MAE $\approx 0.032^{\circ}$，CNN MAE $\approx 0.08^{\circ}$，即在该设定下 FFT 的平均误差更低。早期将 FFT 直接用于 $128{\times}128$ 测试裁剪时曾出现数度量级的伪高误差，属评测口径问题而非物理必然；正文第~3~章对两种口径予以区分。Distortion sweep 等实验仍用于刻画不同畸变强度下的相对表现，为真实数据迁移提供补充。

实验方面，已完成 MoS$_2$ 块材机械剥离与 SiO$_2$/Si 衬底转移，正在进行单层区域的光学显微鉴别，后续将开展转角堆叠制备、AFM 形貌表征及 SHG 转角定标验证；实验数据拟采用 Cypher ES 输出的 \texttt{.ibw} 等格式，经平面校正与行对齐后接入同一 pipeline。
...
Twisted two-dimensional material systems form moiré superlattices through interlayer rotation, exhibiting flat bands and strongly correlated phases near specific ``magic angles''. Twisted MoS$_2$ homojunctions are a representative platform where physical properties depend critically on twist-angle accuracy. FFT-based angle estimation is accurate when frequency resolution is sufficient and distortions are mild, but peak picking can fail under a small field of view or heavy distortion. Convolutional networks offer end-to-end regression on a fixed input size and can be combined with MC Dropout for uncertainty estimates.

This work implements a simulation-to-learning pipeline: moiré envelopes from a reciprocal-space difference construction and continuous lattice reconstruction ($\mathrm{strength} = \mathrm{clip}(1 - \theta/2, 0, 1)$). The synthetic dataset emulates tapping-mode multichannel AFM stacks (height / phase / amplitude) at $128{\times}128$ pixels with $\mathcal{O}(10^4)$--$\mathcal{O}(10^5)$ samples, including probe convolution, affine drift, background tilt, scan-line noise, Gaussian blur, $1/f$ noise, and feedback ringing aligned with Cypher-class AFM usage. The CNN uses a modified ResNet (e.g.\ $3{\times}3$ stem, no inappropriate max pooling, Huber loss with $\delta=0.02$, Mixup), optional FFT-magnitude side channels, and MC Dropout at inference.

Fair evaluation protocol: the CNN always sees $128{\times}128$ crops consistent with training, while FFT is run on re-synthesized $512{\times}512$ height maps under the same physics and degradation so that FFT has adequate $k$-space resolution. Representative numbers from the project script \texttt{eval\_compare.py} (v3) on $N{=}5000$ test angles are FFT MAE $\approx 0.032^{\circ}$ and CNN MAE $\approx 0.08^{\circ}$, i.e.\ FFT is more accurate under this matched-resynthesis setting. Earlier reports of multi-degree FFT errors on $128{\times}128$ crops reflected an unfair FFT baseline, not a fundamental limitation of FFT on high-resolution data. Distortion-sweep experiments still characterize relative robustness under varying affine strength.

Experimentally, exfoliation and transfer to SiO$_2$/Si are done; monolayer search, twist stacking, AFM imaging, and SHG calibration follow. Experimental stacks are expected in Cypher ES \texttt{.ibw} form and will be flattened and fed into the same code path after validation on real data.
...
表~\ref{tab:moire_params} 给出不同转角下的 moiré 参数及 FFT 理论不确定度（每 moiré 周期 20 像素）。

\begin{generaltab}{MoS$_2$ 不同转角下的 moiré 参数（$a = 0.316\,\mathrm{nm}$）}{tab:moire_params}
  \begin{tabularx}{\textwidth}{CCCC}
    \toprule
    转角 $\theta$ ($^{\circ}$) & moiré 周期 $L$ (nm) & 视野 fov (nm) & 理论 $\delta\theta$ ($^{\circ}$) \\
    \midrule
    0.5 & 31.4 & 628.0 & $\pm 0.025$ \\
    1.0 & 15.7 & 314.0 & $\pm 0.050$ \\
    2.0 & 7.8  & 157.0 & $\pm 0.100$ \\
    3.0 & 5.2  & 104.7 & $\pm 0.150$ \\
    5.0 & 3.1  & 62.8  & $\pm 0.249$ \\
    \bottomrule
  \end{tabularx}
\end{generaltab}
...
\begin{generalfig}[htbp]{不同转角 MoS$_2$ moiré 图案仿真（$\theta = 0.5^{\circ},\, 1.0^{\circ},\, 2.0^{\circ},\, 3.0^{\circ},\, 5.0^{\circ}$，固定视野 314 nm）}{fig:moire_gallery}
  \IfFileExists{../../angle_cnn/outputs/moire_gallery.png}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/outputs/moire_gallery.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{moire\_pipeline.py} 生成 \texttt{moire\_gallery.png}\\vspace{2cm}}}}
\end{generalfig}
...
(2) 大角度（$\theta > 2^{\circ}$）：moiré 周期缩短至 $8\,\mathrm{nm}$ 以下，与普通 AFM 针尖半径（$8$--$10\,\mathrm{nm}$）同量级，针尖卷积将 moiré 图案抹平。$\theta > 3^{\circ}$ 时实验中几乎无法用 AFM 成像 moiré，需借助 STEM 或 STM。因此本文 CNN 的实验验证目标限定在 $\theta = 0.5^{\circ}$--$1.5^{\circ}$，该范围 AFM 可靠成像（周期 $20$--$31\,\mathrm{nm}$），与仿真训练数据的物理假设自洽。
...
代码实现（\texttt{dataset\_generator.py}）默认生成约 $5\times10^4$ 个样本（可调），输出 $128{\times}128$ 像素、轻敲模式三通道（height / phase / amplitude），$\theta$ 在 $[0.5^{\circ},\,5.0^{\circ}]$ 采样后按 8:1:1 划分。退化链按成像物理顺序施加，除表~\ref{tab:degradation} 所列外，还包括 TITAN~70 类探针卷积（可变针尖半径至约 $7\,\mathrm{nm}$）、$1/f$ 噪声、反馈振荡与扫描方向偏移等，以贴近 Cypher ES + Tapping mode 实验。图~\ref{fig:dataset_preview} 为典型样本预览（运行脚本后存于 \texttt{data/dataset\_preview.png}）。
...
采用改造 ResNet 骨干（默认 ResNet-18，可选 ResNet-34/50），主要改动包括：(1) 输入为与数据集一致的多通道（1 或 3 通道，可选附加 FFT 幅值通道）；(2) 首层卷积为 $3{\times}3$、步长 2，移除不当的 max pooling，避免 $128{\times}128$ 输入过早损失 moiré 高频；(3) 回归头为全连接 + Dropout（如 $512\to256\to64\to1$），输出层无 Sigmoid 束缚，直接在 $[\theta_{\min},\theta_{\max}]$ 上回归；(4) 可选 \texttt{--fft-channel} 将 $\log$ FFT 幅值作为额外输入通道。参数量随骨干与是否附加通道变化，数量级为 $10^7$。

\subsection{训练策略}

损失函数为 Huber Loss（$\delta = 0.02$，与 \texttt{train\_cnn.py} v4 默认一致），优化器 AdamW，学习率 Warmup + Cosine 衰减，训练集上使用 Mixup；可选 BF16 混合精度、\texttt{torch.compile} 加速。早停按验证集 loss 监控。推理阶段可对 Dropout 多次前向（MC Dropout）得到均值与不确定性。
...
\begin{generalfig}[htbp]{CNN 训练曲线：Loss 与验证集指标（由脚本导出）}{fig:train_log}
  \IfFileExists{../../angle_cnn/outputs/train_log.png}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/outputs/train_log.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{train\_cnn.py} 生成 \texttt{train\_log.png} 后重新编译\vspace{2cm}}}}
\end{generalfig}
...
\begin{generalfig}[htbp]{数据集多通道样本预览（示例文件 \texttt{dataset\_preview.png}）}{fig:dataset_preview}
  \IfFileExists{../../data/dataset_preview.png}{%
    \includegraphics[width=\textwidth]{../../data/dataset_preview.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{dataset\_generator.py} 生成 \texttt{dataset\_preview.png}\vspace{2cm}}}}
\end{generalfig}
...
\begin{generalfig}[htbp]{FFT（左）与 CNN（右）预测值 vs 真实值（公平对比脚本输出）}{fig:compare_scatter}
  \IfFileExists{../../angle_cnn/outputs/compare_scatter.png}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/outputs/compare_scatter.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{eval\_compare.py} 生成 \texttt{compare\_scatter.png}\vspace{2cm}}}}
\end{generalfig}
...
\begin{generalfig}[htbp]{误差分布与各角度区间 MAE 等（\texttt{compare\_error.png}）}{fig:compare_error}
  \IfFileExists{../../angle_cnn/outputs/compare_error.png}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/outputs/compare_error.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{eval\_compare.py} 生成 \texttt{compare\_error.png}\vspace{2cm}}}}
\end{generalfig}
...
脚本 \texttt{distortion\_sweep.py} 在固定其它退化量的前提下扫描仿射畸变强度，比较 FFT 与 CNN 的误差曲线（实现细节见仓库说明）。需注意：即便在部分畸变区间内 CNN 相对更稳，也不能与第~\ref{tab:final_stats} 节的公平 MAE 表混为一谈——二者扫描维度与汇总方式不同。结果如图~\ref{fig:distortion_sweep}。

\begin{generalfig}[htbp]{Distortion Sweep：FFT vs CNN 随畸变强度变化（由 \texttt{distortion\_sweep.py} 导出）}{fig:distortion_sweep}
  \IfFileExists{../../angle_cnn/outputs/distortion_sweep.png}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/outputs/distortion_sweep.png}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}请运行 \texttt{distortion\_sweep.py} 生成 \texttt{distortion\_sweep.png}\vspace{2cm}}}}
\end{generalfig}
...
为避免本文仅停留于仿真，本工作将已有扫描探针文献作为外部实证参照。Liao 等在大面积 MoS$_2$ 同质结构工作中给出了 AFM 形貌图（见原文 Fig.~2），用于验证样品表面与界面质量\cite{liao2020precise}，说明 MoS$_2$ 扭转体系具备 AFM 形貌表征的实验可行性。McGilly 等采用压电响应力显微镜（PFM，AFM 家族模式）直接可视化了 moiré 超晶格的实空间网络\cite{mcgilly2020visualization}。Engelke 等提出扭转力显微（TFM）并实现了 van der Waals moiré 与原子晶格成像\cite{engelke2024torsional}。
...
\begin{generalfig}[htbp]{文献实证图像示例 I：Nano Lett. 2023（STM）中不同扭角下的 moiré 纹理与频域峰，展示了与本文 FFT 管线一致的“实空间周期 + 频域峰”信息}{fig:lit_stm_moire_fft}
  \IfFileExists{../../angle_cnn/docs/figures/literature/pmc10603799/nanolett2023_fig1_stm_fft.jpg}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/docs/figures/literature/pmc10603799/nanolett2023_fig1_stm_fft.jpg}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}缺少文献图像文件：\texttt{nanolett2023\_fig1\_stm\_fft.jpg}\vspace{2cm}}}}
\end{generalfig}

\begin{generalfig}[htbp]{文献实证图像示例 II：Nat. Commun. 2020（AFM）中 MoS$_2$ 扭转样品形貌（Fig.~2），可作为本文 AFM 可行性的直接实验先例}{fig:lit_afm_liao}
  \IfFileExists{../../angle_cnn/docs/figures/literature/pmc7195481/liao2020_fig2_afm.jpg}{%
    \includegraphics[width=\textwidth]{../../angle_cnn/docs/figures/literature/pmc7195481/liao2020_fig2_afm.jpg}}{%
    \fbox{\parbox{\textwidth}{\centering\vspace{2cm}缺少文献图像文件：\texttt{liao2020\_fig2\_afm.jpg}\vspace{2cm}}}}
\end{generalfig}

上述图像用于“可观测性与方法可行性”论证，而非作为本文自采样品结果。与 STM 相比，AFM 在原子/电子态细节上的对比度通常较弱，但在 moiré 周期、域结构趋势与频域峰可辨性层面仍可提供与本文算法目标一致的关键信息。

需要说明的是，上述文献与本文任务并非完全同构：本文聚焦于 Tapping 模式下 Height / Phase / Amplitude 通道的转角提取 pipeline。当前阶段属于“仿真与算法先行”，后续将以真实 AFM 数据完成迁移验证，并与 SHG 转角定标进行交叉比对。
...
\subsection{连续晶格重构模型（moire\_pipeline.py 节选）}

\begin{verbatim}
# 连续晶格重构（无 if 硬切换）
# strength = clip(1 - θ/2, 0, 1)：θ=0°→1.0，θ≥2°→0.0
...
\begin{verbatim}
MoS₂ moiré：FFT vs CNN 角度提取公平对比报告
==================================================

材料：MoS₂ 转角同质结（a = 0.316 nm）
对比方法：
...
CNN 比 FFT 精度提升：0.5 倍
（注：该比值定义为 MAE_FFT/MAE_CNN；公平口径下 FFT 更优时小于 1。）
