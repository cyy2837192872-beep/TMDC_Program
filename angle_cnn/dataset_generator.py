...
from angle_cnn.core.config import (
    A_NM, DEFAULT_PPP, IMG_SIZE, SIM_SIZE, THETA_MIN, THETA_MAX,
...
from angle_cnn.core.physics import moire_period, pixels_per_moire_period
from angle_cnn.core.fonts import cjk_fontproperties
...
def parse_args():
    ...
    parser.add_argument("--theta-sampling", type=str, default="uniform",
                        choices=["uniform", "log", "inv_square"],
                        help="θ 采样分布: uniform=均匀, log=对数均匀(小角度密集), "
                             "inv_square=1/θ²(更激进的小角度优先)")
    parser.add_argument("--log-theta", action="store_true", dest="_deprecated_log_theta",
                        help="已弃用: 请用 --theta-sampling log 代替")
...
def generate_dataset(
    ...
    sampling_label = {"uniform": "线性均匀", "log": "对数均匀（小角度密集）",
                       "inv_square": "1/θ²（小角度高度优先）"}[theta_sampling]
    print(f"θ 采样：{sampling_label}")
    ...
    if theta_sampling == "log":
        thetas = np.exp(np.linspace(
            np.log(theta_min), np.log(theta_max), n_total
        ))
    elif theta_sampling == "inv_square":
        # p(θ) ∝ 1/θ² → CDF: F(θ) = (1/θ_min - 1/θ) / (1/θ_min - 1/θ_max)
        inv_min = 1.0 / theta_min
        inv_max = 1.0 / theta_max
        u = np.linspace(0.0, 1.0, n_total)
        thetas = 1.0 / (inv_min - u * (inv_min - inv_max))
    else:
...
def main():
    ...
    # Handle deprecated --log-theta flag
    theta_sampling = args.theta_sampling
    if args._deprecated_log_theta:
        import warnings
        warnings.warn("--log-theta 已弃用，请用 --theta-sampling log 代替", FutureWarning)
        theta_sampling = "log"
    sampling_label = {"uniform": "线性均匀", "log": "对数均匀（小角度密集）",
                       "inv_square": "1/θ²（小角度高度优先）"}[theta_sampling]
    print(f"  θ 采样:      {sampling_label}")
