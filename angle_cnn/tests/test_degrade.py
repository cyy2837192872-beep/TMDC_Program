...
class TestAffineDistortion:
    def test_identity(self):
        from core.degrade import apply_affine_distortion
...
class TestGaussianNoise:
    def test_shape_preserved(self):
        from core.degrade import apply_isotropic_gaussian_noise
...
class TestBackgroundTilt:
    def test_shape_preserved(self):
        from core.degrade import apply_background_tilt
        img = np.random.randn(64, 64).astype(np.float32)
...
class TestGaussianBlur:
    def test_shape_preserved(self):
        from core.degrade import apply_gaussian_blur
        img = np.random.randn(64, 64).astype(np.float32)
...
class TestTipConvolution:
    def test_zero_radius(self):
        from core.degrade import apply_tip_convolution
        img = np.random.randn(64, 64).astype(np.float32)
