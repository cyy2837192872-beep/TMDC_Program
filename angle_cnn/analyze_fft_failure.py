...
def main():
    ...
    print("=" * 70)
    print("""
【根本原因】FFT 在 128×128 小图上误差变大的主要原因：

1. **频率分辨率（视野）缩小**
   - 中心裁剪后物理视野约为全图的 1/4：fov_128 ≈ fov_512/4
   - FFT 频率步长 δf ∝ 1/fov，故峰位分辨变差；由式 δθ = a√3/(2·fov)，角度不确定度随 fov 缩小而增大（约 4 倍）

2. **每周期像素数 ppp 与峰半径**
   - 在固定 nm 视野下，ppp = N·L/fov；裁剪后 N 与 fov 同比缩小，**同一 θ 下 ppp 数值不变**
   - r_peak = N/ppp = fov/L（nm 意义下由莫尔周期决定）；小图上仍是同一物理周期，但**整幅图所含周期数变少**，峰更易与噪声/边界混淆

3. **采样与混叠**
   - 小裁剪所含完整莫尔周期更少，谱峰统计变差；若退化较强，小图更易丢峰

【解决方案】

选项A：评估 FFT 时使用完整 512×512 图像（或与训练解耦的高分辨率重算图）
  - 优点：FFT发挥最佳性能
